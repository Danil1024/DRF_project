...
class CategogyTestCase(TestCase):
	def test_create(self):
		...
		expected_category_data = (1, 'category_1')
	...
	def test_some_names(self):
		try:
			...
			category_2 = Category.objects.create(title='category')
		except IntegrityError as error:
			self.assertEqual('UNIQUE constraint failed: women_category.title', str(error))

	def test_delete(self):
		category = Category.objects.create(title='category_1')
		category.delete()
		received_category = Category.objects.filter(title='category_1')
	...
	def test_read_list(self):
		...
		expected_category_data = [(1, 'category_1'),(2, 'category_2')]
		self.assertEqual(expected_category_data, received_categories_data)

	def test_read_detail(self):
		category_1 = Category.objects.create(title='category_1')
		category = Category.objects.get(title='category_1')
...
class WomenTestCase(TestCase):
	...
	def test_read_list(self):
		...
		expected_womens_data = [(1, 'women_1', None, user.id), (2, 'women_2', None, user.id)]
	...
	def test_update(self):
		user_1 = User.objects.create_user(
			username='user_1', password='1q2w3e4r5tQ')
		...
		user_2 = User.objects.create_user(
			username='user_2', password='1q2w3e4r5tQ')
